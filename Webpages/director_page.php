<?php 
    ...
    $dir = $_GET['director']; //Gets the director_id from the url


    //Begin getting information about the requested movie
    // Desired Query goes in the " ".
    $query = "SELECT *, CONCAT(fname, ' ', lname) as name FROM directors WHERE director_id = $dir";
    /*
    ...
    */
    ...
    while($row = mysqli_fetch_assoc($db)){
        ...
        $name = $row['name'];
        $bio = $row['bio'];
        $movies_dir = $row['movies_directed'];
                ...
                <div id="info">
                   <h2>Biography:</h2>
                    ...
                    <h2>Movies Directed</h2>
                        <h4><?php
                        $dir_query = "SELECT movies.movie_id, title FROM movies JOIN $movies_dir ON movies.movie_id = $movies_dir.movie_id";
